...
public class DiskMapStoreKeyClosableIterator  implements ClosableIterator<ByteArray>  {
	
	private DiskMapStore diskMapStore ;
	private short currentDiskMapManager = 0 ;
	private DiskMapKeyClosableIterator currentManagerIterator ;
	
	public DiskMapStoreKeyClosableIterator(DiskMapStore diskMapStore) throws VoldemortException {
	...
	private boolean openNextIterator() {
		...
		if ( currentDiskMapManager >= diskMapStore.getNbZones() )
			return false ;
		currentManagerIterator = new DiskMapKeyClosableIterator(diskMapStore.getDiskMapManager(currentDiskMapManager)) ;
		return true ;
	}
